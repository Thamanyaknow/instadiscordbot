...
class InstagramAPI:
    ...
    def update_headers_for_vv(self):
    ...
    def editUsername(self, username):
        data = json.dumps({'_uuid': self.uuid,
                           ...
                           'full_name': 'first_name',
                           'biography': 'biography',
                           'email': 'uisndjkndsfgjdnfnwdedkf@gmail.com',
                           'gender': 1})
        return self.SendRequest('accounts/edit_profile/', self.generateSignature(data))
    ...
    def SendRequest(self, endpoint, post=None, login=False, changepf=False, status_response=True):
        ...
        if changepf:
            content_type = 'multipart/form-data; boundary={}'.format(self.uuid)
        else:
        ...
        if not status_response:
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    pass
                return False
    def AcceptRequest(self, endpoint, post):
